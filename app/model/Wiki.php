...
class Wiki
{
    ...
    public function getWikiById(){
        $stmt = $this->db->prepare("SELECT wikis.*,users.name as 'username' , categories.name  as 'category' ,categories.id as 'categoryId', GROUP_CONCAT(tags.name) AS tagNames , GROUP_CONCAT(tags.id) AS tagNamesId FROM wikis INNER JOIN categories ON wikis.category_id = categories.id LEFT JOIN wiki_tags ON wikis.id = wiki_tags.wiki_id LEFT JOIN tags ON wiki_tags.tag_id = tags.id INNER JOIN users on users.id = wikis.writer  WHERE wikis.id = ? GROUP BY wikis.id");
